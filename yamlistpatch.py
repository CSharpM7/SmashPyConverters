...
def findSource():
    if root.modFile == "":
        return
    filePath = root.modFile[root.modFile.index("fighter/"):]
    toReturn = root.arcDir+"/export/"+filePath
...
def startGUI():
    ...
    source_sep = Frame(sourceFrame,width = 4)
    source_sep.pack(side = RIGHT)
    source_btn = Button(sourceFrame, text = "Set",width=7, command=setSource)
    ...
    #create run button
    run_btn = Button(root, text="Run", command=run,anchor=S,width=20)
    run_btn.pack(side = BOTTOM,pady=8)
...
def run():    
    ...
    patchFile = root.modFile.replace(".bin",".motdiff")

    subcall = ["yamlist.exe","diff",root.sourceFile,root.modFile,"-l","Labels.txt","-o",patchFile]
